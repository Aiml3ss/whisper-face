...
enter a release. Unsigned previews are useful for pipeline testing but must not
be published as production builds.
...
ticket, computes digests only after stapling, then re-verifies every manifest
artifact. Missing or partial credentials, a non-accepted status, or reported
notary issues abort before a production trust claim can be written. This follows
...
4. Require the macOS release workflow to pass signing, notarization, stapling,
   manifest verification, and checksum generation.
...
and `learned.json` into it with user-only permissions. Copy `transcripts.jsonl`
only if retaining transcript history is intentional. Review the files rather
than copying the entire old checkout or its logs.
