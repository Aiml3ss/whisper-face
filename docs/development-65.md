...
| 6 | VoiceOver, keyboard, contrast, reduced motion, localization readiness | active | VoiceOver metadata, system colors, Reduce Motion, native Tab loops, key equivalents, locale propagation, and catalog-owned primary Overview, Settings, onboarding, Results, Models, Diagnostics, degraded, enum, and accessibility copy are covered; remaining validation, operation-failure, and verification-result messages must move behind semantic keys before additional locales are safe |
...
| 22 | Inspectable personal acoustic keyword memory | active | A strict bounded v1 store requires three unique observations plus two confirmations, uses salted hashed app scopes, supports count-only export and exact/all forget, and retains no raw audio/context/transcript history; caller persistence, UI, and recognizer integration remain intentionally absent |
...
| 25 | Delayed cleanup merges only untouched VoiceIR spans | active | A pure three-way algorithm preserves user edits, applies only uniquely anchored untouched proposal spans, rejects touched boundaries, duplicates, reorderings, overlaps, and full replacements, and returns explainable candidates without writes; transactional destination read/apply integration remains |
