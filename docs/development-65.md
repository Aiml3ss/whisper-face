...
| 22 | Inspectable personal acoustic keyword memory | active | Both installers now create/preserve a private strict v1 store; malformed state fails closed, runtime status exposes only bounded counts/hashed scopes, and explicit export plus exact/all forget atomically persist without raw audio/context/transcript history; no trusted observation source, GUI, or recognizer authority exists yet |
