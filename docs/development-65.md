...
| 6 | VoiceOver, keyboard, contrast, reduced motion, localization readiness | active | VoiceOver metadata, system colors, Reduce Motion, native Tab loops, key equivalents, locale propagation, and catalog-owned Settings, onboarding, Results, Models, Diagnostics, degraded, enum, and accessibility copy are covered; remaining Overview and runtime-provided status copy must move behind semantic keys before additional locales are safe |
